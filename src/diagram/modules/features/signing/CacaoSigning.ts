 ...
 */
const hashAlgorithmList = ['sha256', 'sha224', 'sha384', 'sha512'];

/**
 * list of the sign algorithms supported by the application
 */
const signAlgorithmList = ['RSA-SHA256', 'RSA-SHA384', 'RSA-SHA512'];
...
export default class CacaoSigning {
	...
	showSignatureCheckDialog() {
		const createSignatureTile = (signature: Signature): HTMLDivElement => {
			const createCountersignTile = (countersignature: Signature): HTMLDivElement => {
		...
		const signatureList = this._playbookHandler.playbook.signatures;
		...
		const okButton = document.createElement('button') as HTMLElement;
	...
	async signOrCountersignPlaybook() {
		const action = this.canUserSignOrCounterSignPlaybook();
		...
		if (action === 'sign') {
			...
			for (const signature of this._playbookHandler.playbook.signatures) {
	...
	private createSignatureObject(
		...
		const errorMessage = document.createElement('div') as HTMLElement;
		errorMessage.innerText = '';
		errorMessage.className = 'cacaoDialog__errormessage';
		buttonContainer.appendChild(errorMessage);

		const cancelButton = document.createElement('button') as HTMLElement;
		cancelButton.innerText = 'cancel';
		cancelButton.className = 'buttonList__button button--secondary';
		buttonContainer.appendChild(cancelButton);

		const confirmButton = document.createElement('button') as HTMLElement;
		...
		return new Promise<boolean>(resolve => {
			confirmButton.addEventListener('click', async () => {
				const signature: Signature = new Signature(propertyPanel.submit());
	...
	private countersign(signature: Signature, counterSignature: Signature, privateKey: string) {
		...
		const oldSignatures = this._playbookHandler.playbook.signatures;
		...
		const jsc_canonical = canonicalize(
		...
		const hashValue = hash(counterSignature.hash_algorithm, jsc_canonical);
		...
		const signValue = sign(counterSignature.algorithm, privateKey, Buffer.from(hashValue));
	...
	private async sign(signature: Signature, privateKey: PrivateKey) {
		...
		// Step 5: Create a hash based on the hash algorithm defined in the signature object (e.g., sha-256 in hex) of the JCS version of the entire playbook from step 4
		const hashValue = hash(signature.hash_algorithm, jsc_canonical);
		if (hashValue === undefined) {
			this._playbookHandler.playbook.signatures = oldSignatures;
			throw Error('error during hashing of the playbook');
		}
		...
		// Step 6: Sign the hash from step 5 using the algorithm (e.g., RS256) defined in the signature object and base64URL.encode it
		const signValue = sign(signature.algorithm, privateKey, Buffer.from(hashValue));
		// Step 7: Append the new b64 digital signature from step 6 to the signatures value property (also include any existing signatures, that were removed in step 2)
	...
	private verifySignature(
		...
		const playbookSignatures = this._playbookHandler.playbook.signatures;
		...
		for (let playbookSignature of this._playbookHandler.playbook.signatures) {
			...
			while (countersign) {
				...
				const temp = new Signature(countersign.signature);
...
function sign(algorithm: string, private_key: string, value: Buffer): Buffer {
	...
	const buffer = signature.sign(private_key);
	return buffer;
}
