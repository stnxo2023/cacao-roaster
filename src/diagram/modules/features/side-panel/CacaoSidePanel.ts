...
import type SelectionTool from 'diagram-js/lib/features/selection/Selection';
import type { WorkflowStep } from 'cacao2-js';
...
export default class CacaoSidePanel {
	...
	private _hide() {
		...
		document
			.getElementsByClassName('integration_logs-container')[0]
			?.classList.remove('integration_logs-container--move');
	}
	...
	private _show() {
		...
		document
			.getElementsByClassName('integration_logs-container')[0]
			?.classList.add('integration_logs-container--move');
	}
