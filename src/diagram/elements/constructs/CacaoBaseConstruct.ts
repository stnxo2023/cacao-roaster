...
import type { CacaoConnectionType } from '../connections/CacaoBaseConnection';
...
import type { DrawProps } from '../../modules/draw/DrawFactory';
import { executionStatusColor, executionStatusColorStrong } from '../../modules/model/SchemaTypes';
...
export default abstract class CacaoBaseConstruct {
	...
	getExecutionStatusDotProps(shape: Shape, value: any): DrawProps {
		...
		return {
			...
			styleAttrs: {
				...
				filter: `drop-shadow(0px 0px 1.5px ${strokeColor ?? 'rgb(50,50,50)'})`,
				strokeOpacity: 1,
				strokeWidth: 0.7,
				rx: 10,
				ry: 10,
	...
	getPrincipalShapeProps(
		...
		const background = (shape: Shape): DrawProps => {
			return {
				type: 'rectangle',
				x: 0,
				y: 0,
		...
		const header = (shape: Shape): DrawProps[] => {
			const headerBackgroundProps1: DrawProps = {
				type: 'rectangle',
				x: 0,
				y: 0,
				width: shape.width,
			...
			const headerBackgroundProps2: DrawProps = {
				type: 'rectangle',
				x: 0,
				y: 10,
				width: shape.width,
			...
			const headerLabelProps: DrawProps = {
				type: 'text',
				x: 7,
				y: 0,
				width: shape.width,
		...
		const name = (shape: Shape, description: string): DrawProps => {
			return {
				type: 'text',
				x: 5,
				y: 20,
		...
		const border = (shape: Shape): DrawProps => {
			return {
				type: 'rectangle',
				x: 0,
				y: 0,
		...
		return [
			background(shape),
			...header(shape),
			name(shape, CacaoUtils.isDefined(workflowStep?.name) ? workflowStep.name : 'Add name'),
			border(shape),
		];
	}
}
